...
// "the definition is wrong and nothing was sent", and a setup chain that
// authenticated against a real system has already broken that promise.
func TestLoadSetupFailureExitsOneNotTwo(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/login" {
			w.WriteHeader(500)
			return
	...
	writeRequest(t, dir, "auth/login.yaml",
		"name: login\nprotocol: http\nhttp:\n  method: POST\n  url: \""+srv.URL+"/login\"\n"+
			"assertions:\n  - from: status\n    op: eq\n    value: \"200\"\n")
	reqPath := writeRequest(t, dir, "load/ping.yaml",
		"name: ping\nprotocol: http\nhttp:\n  method: GET\n  url: \""+srv.URL+"/x\"\n"+
			"load:\n  rate: 500\n  requests: 20\n")

	_, errOut, code := run(t, "load", reqPath, "--setup", filepath.Join(dir, "auth"))
	require.Equal(t, 1, code)
	require.Contains(t, errOut, "setup")
}

// An output format qv load cannot render is a bad invocation, so it is exit 2 —
// and it is rejected before the report is written, not by failing halfway
// through one.
func TestLoadRejectsUnsupportedOutputFormat(t *testing.T) {
	...
	out, errOut, code := run(t, "load", reqPath, "--rate", "500", "--requests", "10", "--output", "xml")
	require.Equal(t, 2, code)
	require.Contains(t, errOut, "not supported by qv load")
	require.Empty(t, out)
}

// The one line a CI log shows for a failed gate has to name the threshold. This
// path built its exitError around an error that was always nil, so Error()
// dereferenced nil and the message was a formatter panic.
func TestLoadThresholdFailureExplainsItselfOnStderr(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(500)
	}))
	defer srv.Close()
